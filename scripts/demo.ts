/**
 * End-to-end demo: publish → purchase → open (rotate) → share → re-open transfers access.
 * Runs the security server in-process and drives it with the real ViewerClient.
 *
 *   npm run demo
 */
...
async function main(): Promise<void> {
  ...
  step('① 출판사가 문서를 전자서점에 등록');
  const content = Buffer.from('CONFIDENTIAL — Q3 go-to-market strategy.\n재배포 금지.', 'utf8');
  const { doc } = await api(base, '/titles', {
    title: 'Q3 Strategy',
    contentBase64: content.toString('base64'),
    priceCents: 2900,
    ownerId: 'acme-press',
  });
  ok(`등록 — 고유값(doc) ${short(doc)}`);

  step('② Alice가 구매 후 다운로드 (결제가 다운로드의 선행조건)');
  const denied = await fetch(`${base}/download`, {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
    body: JSON.stringify({ userId: 'alice', doc }),
  });
  ok(`구매 전 다운로드 시도 → HTTP ${denied.status} (거부됨)`);
  await api(base, '/purchase', { userId: 'alice', doc });
  const dl = await api(base, '/download', { userId: 'alice', doc });
  ...
  step('④ Alice가 그 .crypton 파일을 Bob에게 공유');
  let bobFile = asFile(aliceFile);
  ok('Bob이 동일 파일(토큰 T2 내장)을 수신');

  step('⑤ Bob이 문서를 엶 → 인증 성공, 토큰 회전 (T2→T3)');
  ...
  step('⑧ 감사 로그 (copy 단위)');
  const log = await api(base, `/audit/copy/${aliceFile.manifest.copyId}`);
  for (const e of log) {
